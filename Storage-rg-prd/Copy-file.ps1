...
get-azureRmstorageaccount -resourcegroupName $resourceGroupName | where-object {$_.storageaccountname -like "storappsinterne*"} | foreach-object { 
    ...
    Get-azureStorageAccountKey -ResourceGroupName $_.resourcegroupname -Name $_.StorageAccountName } | where-object { $_.keyname -like "key1"} | ForEach-Object { 
...
$SourceKey = (Get-azureStorageAccountKey -Name storappsinterneprd -ResourceGroupName storage-rg-prd | where-object {$_.keyname -eq "key1"}).value
$DestKey   = (Get-azureStorageAccountKey -Name storappsinterne$Environnement -ResourceGroupName $resourceGroupName | where-object {$_.keyname -eq "key1"}).value
