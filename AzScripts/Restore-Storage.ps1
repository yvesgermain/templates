...
function restore-storage {
    ...
    if (!( get-AzStorageContainer -context $context.context | where-object { $_.name -like "$container-$Source-$date*" } -ErrorAction SilentlyContinue)) {
        write-warning "Le backup $container-$Source-$date n'existe pas";
        break 
    }

    if (!$date) {
        $dateformat = ((get-AzStorageContainer -context $context.context | Where-Object { $_.name -like "$container-$Source-*" } | sort-object -Property LastModified -Descending)[0]).LastModifiedtime.ToLocalTime()
